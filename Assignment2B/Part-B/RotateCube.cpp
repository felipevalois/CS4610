...
void mykey(GLFWwindow *window, int key, int scancode, int action, int mods)
{
   if (action == GLFW_PRESS)
   {
      ...
      //Rotate around Y axis - no middle button on MacBook?
      if (key == GLFW_KEY_Y)
      {
         Axis = Yaxis;
      }
      ...
      //Press the key 'w' to move the cube upward in the y axis.
      if (key == GLFW_KEY_W)
      {
         Dim = Ydim;
         translation = 0.001f;
      }
      //Press the key 'x' to move the cube downward in the y axis.
      if (key == GLFW_KEY_X)
      {
         Dim = Ydim;
         translation = -0.001f;
      ...
      //Press the key 'f' to move the cube forward in the z axis.
      if (key == GLFW_KEY_F)
      {
         Dim = Zdim;
         translation = 0.001f;
      }
      //Press the key 'b' to move the cube backward in the z axis.
      if (key == GLFW_KEY_B)
      {
         Dim = Zdim;
         translation = -0.001f;
...
int main(int argc, char **argv)
{
   ...
   #endif
   window = glfwCreateWindow(640, 640, "RotateCube", NULL, NULL);
